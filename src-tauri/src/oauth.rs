...
fn fetch_jwks_cached(jwks_uri: &str) -> Result<Value, Box<dyn Error>> {
    ...
    let jwks: Value =
        serde_json::from_str(&resp_text).map_err(|e| Box::new(e) as Box<dyn Error>)?;
...
pub fn verify_id_token(
    ...
    let header = decode_header(id_token).map_err(|e| Box::new(e) as Box<dyn Error>)?;
    ...
    let n = jwk.get("n").and_then(|v| v.as_str()).ok_or_else(|| {
        Box::new(IoError::new(ErrorKind::Other, "JWK missing 'n'")) as Box<dyn Error>
    })?;
    let e = jwk.get("e").and_then(|v| v.as_str()).ok_or_else(|| {
        Box::new(IoError::new(ErrorKind::Other, "JWK missing 'e'")) as Box<dyn Error>
    })?;

    let decoding_key =
        DecodingKey::from_rsa_components(n, e).map_err(|e| Box::new(e) as Box<dyn Error>)?;
    ...
    if let Some(expected_nonce) = expected_nonce {
        match &claims.nonce {
            ...
            _ => return Err(Box::new(IoError::new(ErrorKind::Other, "nonce mismatch"))),
        }
    }
...
pub fn validate_access_token(access_token: &str) -> Result<Option<ValidateResp>, Box<dyn Error>> {
    let client = BlockingClient::new();
    let resp = client
...
pub fn refresh_access_token(refresh_token: &str) -> Result<(), Box<dyn std::error::Error>> {
    dotenvy::dotenv().ok();
    ...
    if status.as_u16() == 401 {
        ...
        let _ = Entry::new("notisr", "refresh_token").and_then(|e| e.delete_credential());
        return Err(Box::new(std::io::Error::new(
            std::io::ErrorKind::PermissionDenied,
